...
module.exports = async (client, emoji) => {
    ...
    let eC = db.get(`eC_${emoji.guild.id}`)
    if(eC === null) {
      return;
    }
    ...
    emoji.guild.fetchAuditLogs().then(logs => {
		...
		.addField(language("EC_FIELD_ADD"), `${language("EMOJIS_FIELD_VALUE_NAME")}: ${emoji.name}\n${language("EMOJIS_FIELD_VALUE_ANIMATED_BOOLEAN")}: ${typeEmoji}`)
		...
		emoji.guild.channels.cache.get(eC).send(msgEC)
	});
}